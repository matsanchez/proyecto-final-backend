  ...
  async create(producto) {
    try {
      ...
      if (fs.existsSync(this._pathToFile)) {
        ...
        await fs.promises.writeFile(this._pathToFile, JSON.stringify(productos, null, 2));
        return { status: "success", mensaje: "Producto Agregado", producto };
      } else {
        producto.id = 1;
        producto.timestamp = Date.now();
        await fs.promises.writeFile(this._pathToFile, JSON.stringify([producto], null, 2));
        return { status: "success", mensaje: "Producto Agregado", producto };
      }
    } catch (error) {
  ...
  async deleteById(id) {
    ...
    if (fs.existsSync(this._pathToFile)) {
      ...
      if (validation) {
        ...
        await fs.promises.writeFile(this._pathToFile, JSON.stringify(newProductosArray, null, 2));
