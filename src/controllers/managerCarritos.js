  ...
  async create() {
    try {
      ...
      if (fs.existsSync(this._pathToFile)) {
        ...
        carrito.timestamp = Date.now();
        carrito.productos = [];
        await fs.promises.writeFile(this._pathToFile, JSON.stringify([carrito], null, 2));
        return { status: "success", mensaje: "Carrito agregado", carrito };
      }
    } catch (error) {
  ...
  async deleteById(id) {
    if (fs.existsSync(this._pathToFile)) {
      ...
      if (validacion) {
        ...
        await fs.promises.writeFile(this._pathToFile, JSON.stringify(newCarritosArray, null, 2));
